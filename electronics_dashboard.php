...
$countApproved = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE status = 'Approved' and branch='Electronics Engineering'")->fetchColumn();
$countRejected = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE status = 'Rejected'and branch='Electronics Engineering'")->fetchColumn();
$countPending = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE status = 'Pending'and branch='Electronics Engineering'")->fetchColumn();
$totalRequests = $pdo->query("SELECT COUNT(*) FROM bonafide_requests where  branch='Electronics Engineering'")->fetchColumn();

// Fetch requests by status
function fetchRequestsByStatus($pdo, $status) {
    if ($status === 'All') {
        $stmt = $pdo->prepare("SELECT * FROM bonafide_requests WHERE branch = 'Electronics Engineering'");
        $stmt->execute();
    } else {
        $stmt = $pdo->prepare("SELECT * FROM bonafide_requests WHERE status = ? AND branch = 'Electronics Engineering'");
        $stmt->execute([$status]);
    }
...
$stmt = $pdo->prepare("SELECT * FROM bonafide_requests WHERE status = 'Pending' AND branch = 'Electronics Engineering'");
$stmt->execute();
...
<script>
    function showData(type) {
        ...
        fetch('fetch_requests_electronics.php?type=' + type)
            .then(response => {
                if (!response.ok) {
    ...
    <table>
        ...
        <?php foreach ($requests as $request): ?>
            <tr id="row-<?= $request['id']; ?>">
                ...
                <td><?= $request['date']; ?></td>
                <td>
                    <form action="view_application_electronics.php" method="POST">
