...
$countApproved = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE status = 'Approved'and branch='Electrical Engineering'")->fetchColumn();
$countRejected = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE status = 'Rejected'and branch='Electrical Engineering'")->fetchColumn();
$countPending = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE status = 'Pending'and branch='Electrical Engineering'")->fetchColumn();
$totalRequests = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE branch='Electrical Engineering'")->fetchColumn();

// Fetch requests by status
function fetchRequestsByStatus($pdo, $status) {
    if ($status === 'All') {
        $stmt = $pdo->prepare("SELECT * FROM bonafide_requests WHERE branch = 'Electrical Engineering'");
        $stmt->execute();
    } else {
        $stmt = $pdo->prepare("SELECT * FROM bonafide_requests WHERE status = ? AND branch = 'Electrical Engineering'");
        $stmt->execute([$status]);
    }
...
$stmt = $pdo->prepare("SELECT * FROM bonafide_requests WHERE status = 'Pending' AND branch = 'Electrical Engineering'");
$stmt->execute();
...
<link rel="stylesheet" href="styles1.css">
    ...
    <script>
        function sendNotification(email,status, rowId) {
            ...
            // Hide the row
            let requestRow = document.getElementById('row-' + rowId);
        if (requestRow) {
            requestRow.remove(); // Completely removes it from the DOM
        }
...
<script>
    function showData(type) {
        ...
        fetch('fetch_requests_electrical.php?type=' + type)
            .then(response => {
                if (!response.ok) {
    ...
    <table>
        ...
        <?php foreach ($requests as $request): ?>
            <tr id="row-<?= $request['id']; ?>">
                ...
                <td><?= $request['date']; ?></td>
                <td>
                    <form action="view_application_electrical.php" method="POST">
