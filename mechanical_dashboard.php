...
$countApproved = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE status = 'Approved' and branch='Mechanical Engineering'")->fetchColumn();
$countRejected = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE status = 'Rejected'and branch='Mechanical Engineering'")->fetchColumn();
$countPending = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE status = 'Pending'and branch='Mechanical Engineering'")->fetchColumn();
$totalRequests = $pdo->query("SELECT COUNT(*) FROM bonafide_requests where branch ='Mechanical Engineering'")->fetchColumn();

// Fetch requests by status
function fetchRequestsByStatus($pdo, $status) {
    if ($status === 'All') {
        $stmt = $pdo->prepare("SELECT * FROM bonafide_requests WHERE branch = 'Mechanical Engineering'");
        $stmt->execute();
    } else {
        $stmt = $pdo->prepare("SELECT * FROM bonafide_requests WHERE status = ? AND branch = 'Mechanical Engineering'");
        $stmt->execute([$status]);
    }
...
$stmt = $pdo->prepare("SELECT * FROM bonafide_requests WHERE status = 'Pending' AND branch = 'Mechanical Engineering'");
$stmt->execute();
...
<script>
    function showData(type) {
        ...
        fetch('fetch_requests_mechanical.php?type=' + type)
            .then(response => {
                if (!response.ok) {
    ...
    <table>
        ...
        <?php foreach ($requests as $request): ?>
            <tr id="row-<?= $request['id']; ?>">
                ...
                <td><?= $request['date']; ?></td>
                <td>
                    <form action="view_application_mechanical.php" method="POST">
