...
$countApproved = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE status = 'Approved' and branch='Computer Engineering'")->fetchColumn();
$countRejected = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE status = 'Rejected'and branch='Computer Engineering'")->fetchColumn();
$countPending = $pdo->query("SELECT COUNT(*) FROM bonafide_requests WHERE status = 'Pending'and branch='Computer Engineering'")->fetchColumn();
$totalRequests = $pdo->query("SELECT COUNT(*) FROM bonafide_requests where  branch='Computer Engineering'")->fetchColumn();

function fetchRequestsByStatus($pdo, $status) {
    if ($status === 'All') {
        $stmt = $pdo->prepare("SELECT * FROM bonafide_requests WHERE branch = 'Computer Engineering'");
        $stmt->execute();
    } else {
        $stmt = $pdo->prepare("SELECT * FROM bonafide_requests WHERE status = ? AND branch = 'Computer Engineering'");
        $stmt->execute([$status]);
    }
...
$stmt = $pdo->prepare("SELECT * FROM bonafide_requests WHERE status = 'Pending' AND branch = 'Computer Engineering'");
$stmt->execute();
...
<script>
    function showData(type) {
        ...
        fetch('fetch_requests_computer.php?type=' + type)
            .then(response => {
                if (!response.ok) {
    ...
    <table>
        ...
        <?php foreach ($requests as $request): ?>
            <tr id="row-<?= $request['id']; ?>">
                ...
                <td><?= $request['date']; ?></td>
                <td>
                    <form action="view_application_computer.php" method="POST">
